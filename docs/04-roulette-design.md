...
- 동일 유저가 잭팟 N회 이상 당첨 → 띄움
- 짧은 시간 내 동일 유저 추첨 폭주 → 띄움 (성공·실패를 가리지 않는다)
- **같은 IP 에서 여러 계정이 잭팟 당첨 → 띄움** (다계정의 전형적인 모양)

셋 다 "부정 판정" 이 아니라 **"봐야 할 것"** 이다. 자동으로 회수하거나 차단하지 않는다 —
오탐 한 건의 비용이 탐지 이득보다 크다.

IP 축에는 전제가 하나 붙는다. 프록시 뒤에서 `X-Forwarded-For` 를 검증 없이 믿으면
누구나 자기 IP 를 위조할 수 있고, 그러면 이 축은 탐지 도구가 아니라
**남에게 혐의를 씌우는 도구**가 된다. 그래서 신뢰할 프록시를 명시적으로 적은 경우에만
헤더를 해석하고(`Network:TrustedProxyNetworks`), 설정이 없으면 IP 축을 **아예 돌리지 않는다**.

설정 없이 돌리면 모든 요청이 프록시 IP 하나로 보여 전원이 이상 징후로 뜨는데,
그 화면을 한 번 본 운영자는 이후 이상 탐지 전체를 신뢰하지 않게 된다.
띄우지 않는 편이 낫고, 화면에는 왜 꺼져 있는지를 대신 적는다.
...
| **7** | 우편 수령·회수 API | ✅ |
| **8** | 확률 변경 2인 승인(maker-checker) | ✅ |
| **9** | 이상 탐지(유저 축) · 부하 측정 하네스 | ✅ |
| **10** | 이상 탐지 IP 축 + 신뢰 프록시 설정 | ✅ |
...
| 우편 수령·회수 | `Application/DrawMailService.cs`, `Api/DrawMailEndpoints.cs` |
| 2인 승인 | `Domain/DrawApproval.cs`, `Application/DrawApprovalService.cs`, `Pages/Draws/{Weights,Approvals}` |
| 이상 탐지 | `Application/DrawAnomalyDetector.cs` |
| 클라이언트 IP 신뢰 경계 | `Application/NetworkOptions.cs` + `Program.cs` (ForwardedHeaders) |
| 부하 하네스 | `loadtest/draw-spike.js`, `provision-draw.sh`, `run-draw.sh` |

### 남은 것

- **부하 측정의 수치** — 하네스는 만들었고 실행마다 초과 지급 0건을 자체 확인한다.
  수치는 실제로 돌려서 채워야 하는 자리다(docs/load-test.md 마지막 절).
- **우편 수령 시 인벤토리 반영** — 우편함까지가 이 시스템의 경계다.
  수령을 확정 짓는 것은 게임 서버의 몫이라 스텁으로 뒀다.
- **외부 재화 서버 연동** — 티켓을 이 시스템이 소유하는 것은 PoC 범위의 선택이다.
  유료 재화라면 5장의 reserve–commit 사가가 먼저다.
- **부정 판정의 자동화** — 탐지는 띄우기만 한다. 회수·차단은 운영자가 누른다.
  오탐 한 건의 비용이 탐지 이득보다 크기 때문이고, 그 판단은 코드가 아니라 제품이 한다.
...
| 한 통의 우편이 두 번 지급되지 않는다 | 동시 10회 수령 → 성공 1건 |
| 수령한 보상은 회수되지 않는다 | 회수 후 수령분은 그대로, 미수령분만 닫힘 |
| 자기 확률 변경을 자기가 통과시킬 수 없다 | 요청자 본인 승인 시도 → 거부 |
| 승인 전에는 확률이 바뀌지 않는다 | 대기 상태에서 가중치·버전 불변 |
| 임계치 미만은 이상 징후로 띄우지 않는다 | 잭팟 2회 → 목록 비어 있음 |
| 같은 IP 의 다계정 잭팟을 띄운다 | 한 IP 3계정 → 탐지, 다른 IP 1계정 → 무시 |
| 믿을 수 없는 IP 로는 IP 축을 돌리지 않는다 | 신뢰 설정 없음 → 같은 IP 4계정이어도 조용 |
