...
| 단계 | 범위 | 산출물 |
|---|---|---|
| **1** | 도메인 + 스키마 + 마이그레이션 | `DrawEvent`, `DrawPrize`, `DrawWeightVersion`, `DrawLog` |
| **2** | 어드민 CRUD — 슬롯 편집기, 확률 미리보기, 시뮬레이터 | Razor Pages 4장 |
| **3** | 워밍업 + Lua 추첨 스크립트 + API | `draw_spin.lua`, `POST /api/draws/{id}/spin` |
| **4** | 아웃박스 워커 + 우편함 + 이력 화면 + 재현 검증 | `DrawPersistenceWorker` |
| **5** | 공시 페이지 + 천장/일일한도/티켓 | `GET /events/{code}/odds` |
| **6** | 편차 모니터링 + 2인 승인 + 부하테스트 | 비교표 갱신 |

1~3 단계만으로 "확률 추첨이 동시성 하에서 정확하다"를 증명할 수 있다. 4 단계부터가 운영 품질이다.
...
1. **소진 정책** — Fallback(권장) / Renormalize 중 무엇을 기본값으로 할 것인가
   → Renormalize 를 택하면 공시 페이지가 실시간이어야 하고 Lua 가 복잡해진다

2. **티켓 재화의 소유권** — 이 시스템이 소유(PoC) / 외부 재화 서버(사가 필요)
   → 후자면 5장의 reserve-commit 패턴을 먼저 만들어야 한다

3. **천장(pity) 포함 여부** — 포함하면 "연속 미당첨"의 정의(꽝만? 잭팟 외 전부?)를 확정해야 한다

4. **지급 경로** — 우편함 경유(권장) / 즉시 인벤토리
   → PoC 는 우편함 테이블까지만, 인벤토리는 스텁

5. **확률 공시 범위** — 전체 경품 / 유료 요소만
   → 무료 이벤트도 공시하면 문의가 줄어든다. 비용은 페이지 하나
