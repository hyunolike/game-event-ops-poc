...
/// <summary>
/// 발급 Lua 스크립트를 EVALSHA 로 실행한다.
/// </summary>
/// <remarks>
/// 스크립트 본문을 매 호출마다 보내면 발급 1건당 수 KB 가 네트워크로 나간다.
/// 초당 수천 건 규모에서는 무시할 수 없으므로 SHA 로만 호출하고,
/// 서버가 스크립트 캐시를 잃은 경우(재시작, SCRIPT FLUSH)에만 재적재 후 한 번 재시도한다.
/// </remarks>
public sealed class IssuanceScript
{
    private readonly IConnectionMultiplexer _mux;
    private readonly string _source;
    private byte[]? _sha;

    public IssuanceScript(IConnectionMultiplexer mux)
    {
        _mux = mux;
        _source = ReadEmbedded("issue_coupon.lua");
    }
